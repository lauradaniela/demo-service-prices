Feature: Endpoint for query prices accord with different parameters
  ...
  Scenario Outline: Client wants to know the prices that apply for the product
    Given A request with aplication date <applicationDate> with the product <idProduct> and brand <idBrand>
  ...
  Scenario Outline:
    Given A request without all mandatory fields aplication date <applicationDate> with the product <idProduct> and brand <idBrand>
    When call the enpoint for check the prices
    Then The system returns the status <responseCode>
    Examples:
    ...
    Scenario Outline:
      Given A request without values that not exists the prices with fields aplication date <applicationDate> with the product <idProduct> and brand <idBrand>
